...
#Unofficial
    ...
    apexqtmo|comanche|d2*|expressatt)
		CONFIG="d2";; # All d2 variants (Galaxy S3), comanche (Galaxy Rugby Pro), apexqtmo (Galaxy S Relay)
                     # and expressatt
	*)
		CONFIG="";; #Not defined
esac;

if [ -n "$CONFIG" ]; then PATH="/sdcard/Synapse/$CONFIG.sh"; else PATH=""; fi;
